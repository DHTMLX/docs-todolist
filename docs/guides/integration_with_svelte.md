...
DHTMLX To Do List is compatible with Svelte. For a working example, see the [Example on GitHub](https://github.com/DHTMLX/svelte-todolist-demo).
...
Create a Svelte component to add To Do List with Toolbar to the application. Create a new file in the *src/* directory and name it *ToDo.svelte*.

#### Import source files

Open *ToDo.svelte* and import To Do List source files. Note that:
...
Open *App.svelte*, import the data, and pass the data to the `<ToDo/>` component as props:
...
Open *ToDo.svelte* and apply the props to the To Do List configuration object:
...
Open *ToDo.svelte* and extend the `onMount()` method:
...
The integration is complete. Customize the code to match your requirements. Find the final example on [GitHub](https://github.com/DHTMLX/svelte-todolist-demo).
