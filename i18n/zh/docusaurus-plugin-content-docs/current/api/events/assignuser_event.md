---
sidebar_label: assign-user
title: assign-user 事件
description: 您可以在 DHTMLX JavaScript To Do List 库的文档中了解 assign-user 事件。浏览开发者指南和 API 参考，查看代码示例和在线演示，并下载免费的 30 天评估版本。
---

# assign-user

### 描述 {#description}

@short: 将用户分配到任务时触发

### 用法 {#usage}

~~~js
"assign-user": ({
    id: string | number,
    userId: string | number
}) => void;
~~~

### 参数 {#parameters}

`assign-user` 事件的回调函数可以接收一个包含以下参数的对象：

- `id` - （必填）任务的 ID
- `userId` - （必填）被分配到该任务的用户的 ID

:::info
要处理内部事件，可以使用 [**Event Bus 方法**](category/event-bus-methods.md)
:::

### 示例 {#example}

~~~js {15-17}
const { ToDo, Toolbar } = todo;
const {tasks, projects, users} = getData();

const list = new ToDo("#root", {
    tasks,
    projects,
    users
});

const toolbar = new Toolbar("#toolbar", {
    api: list.api,
});

// 订阅 "assign-user" 事件
list.api.on("assign-user", ({id, userId}) => {
    console.log("The", userId, "user is assigned to the", id, "task"); 
});
~~~

**相关文章：** [任务用户](guides/task_users.md)
