...
Ознакомившись с кратким обзором To Do List, вы готовы узнать, как отобразить компонент на странице. Следуйте инструкциям статьи [Начало работы](how_to_start.md). Если вы разрабатываете с помощью AI-ассистента, подключите его к актуальной документации через [MCP-сервер DHTMLX](guides/mcp-server.md).
